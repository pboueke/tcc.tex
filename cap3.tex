...
\paragraph{}Para a conexão com o \textit{datalake}, é preciso instalar o conector \textit{ODBC} compatível com o tipo de banco de dados usado no servidor da aplicação. Por fim, é preciso configurar os parâmetros da aplicação contidos no arquivo \textit{config.yaml}, que se encontra no diretório raido projeto, conforme visto na tabela 3.1.

\subsection{Inicialização da aplicação}

\paragraph{}Para a inicialização da aplicação, basta certificar-se de que o banco da aplicação está operacional e executar o comando que inicia a  execução.
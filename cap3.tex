...
\paragraph{}Além da aplicação, existem as componentes \textit{daemon.py} e \textit{datalake.py} que são responsáveis, respectivamente, pela predição de resultados e conexão com o \textit{datalake} onde estão os dados a sofrerem predições. O \textit{datalake} se trata de um serviço de banco de dados externo ao escopo da aplicação, pelo qual são acessados os dados a serem processados. O escalonamento de predições é gerenciado pelo \textit{crontab}, que, por sua vez, é gerenciado pela aplicação.

\paragraph{}A arquitetura como um todo é delimitada pela restrição de que apenas um modelo de aprendizado de máquina é carregado na aplicação. A componente \textit{daemon.py} é responsável pela execução de apenas desse modelo, sendo todas as etapas de configuração e implantação condicionadas a esse fato.
...
\paragraph{}Em aplicações que fazem uso do \textit{framework Django}, a componente \textit{view.py}, vista na figura 3.1, é responsável pelo controle e recebimento das requisições à aplicação. Essa componente é um pacote da linguagem \textit{Python} e contém as funções responsáveis por enviar o HTML a ser visualizado pelo usuário e responder todas as chamadas assíncronas requisitadas pelas interfaces.
...
\paragraph{}Os dois métodos representam dois casos de uso possíveis. O primeiro se trata de quando a \textit{view} é inicializada e todos os casos são inferidos, fazendo uso do método \textit{predict\_all}, o segundo se trata da inferência de apenas um dos casos assincronamente, fazendo uso do método \textit{predict\_one}. Essa \textit{view} tem como objetivo demonstrar a execução do modelo de aprendizado em tempo real, de forma que os efeitos da parametrização dos registros fiquem claros aos usuários.
...
\paragraph{} Essa \textit{view} tem como objetivo disponibilizar uma interface para auditoria dos disparos e predições realizadas, permitindo que os usuários inspecionem todos os dados produzidos pela plataforma.

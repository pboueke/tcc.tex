...
\paragraph{}O projeto desenvolvido se trata de uma aplicação \textit{Django} que conta com três interfaces, uma para execução de um modelo preditivo em ocorrências controladas, outra para monitoramento ddos serviços associados e outra para o disparo escalonado de predições, chamadas, respectivamente, de \textit{Showroom}, \textit{Monitor} e \textit{Disparo}. Essas três interfaces são controladas pela aplicação por meio de três classes, vistas na figura 3.2. Os componentes da aplicação podem ser visualizados na figura 3.1, onde se observa a existência de um arquivo \textit{config.yaml}, responsável por agregar todos os parâmetros e configurações da aplicação.
...
ShowroomUser  & Acesso ã view Showroom       \\ \hline
\end{tabular}%
%}
\end{table}
...
\paragraph{} A componente \textit{datalake.py} é responsável pela conexão com o \textit{datalake} que contém os dados sobre os quais serão feitas as inferências do modelo carregado na componente \textit{daemon.py}. Essa componente depende do conector ODBC e requer a instalação do \textit{driver} requerido pelo tipo do \textit{datalake} a ser acessado. A tabela 3.3 contém os métodos dessa componente.
...
\paragraph{}Além de monitorar os serviços associados ao \textit{framework}, a \textit{view} Monitor também é responsável pelo controle do estado do escalonamento da componente \textit{daemon.py}. Em sua interface, acessível apenas por administradores, é possível adicionar remover o comando de execução do \textit{script daemon.py} do arquivo \textit{crontab}, efetivamente controlando se a execução ocorrerá ou não conforme previsto pela expressão CRON presente no arquivo de configuração nop parâmetro \textit{daemon.cron}, visto na tabela 3.1.
...
\paragraph{}A componente \textit{urls.py} ? respon?avel por descrever as rotas da aplicação. Nela os caminhos que levam às \textit{views} e às chamadas assíncronas estão detalhadas em uma lista, fazendo a associação direta entre as funções da componete \textit{views.py} e as rotas usadas pelo usuário e pela interface da aplicação.                     
